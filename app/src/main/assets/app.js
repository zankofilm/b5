...
// ===== EXPORT ONLY PATCH (NO CORE CHANGE) =====
...
function getLicenseForExport(){
  return state.activation || null;
}

function exportData(){
  return {
    license: getLicenseForExport(),
...
// ================= FINAL FIX: ONE-TIME ACTIVATION + FULL EXPORT =================

// ONE-TIME ACTIVATION SAFE STORE
function saveActivationOnce(payload){
  if(!payload) return;
  state.activation = payload;
  state.activated = true;
  state.activatedAt = state.activatedAt || Date.now();
  saveState();
}

// CHECK ACTIVATION (NON BLOCKING)
function isActivated(){
  return !!state.activated && !!state.activation;
}

// LOAD FULL BENEFICIARIES (ALL MONTHS)
function getAllBeneficiariesForExport(){
  const data = state.beneficiariesByMonth || {};
  let all = [];
...
// FULL ADMIN EXPORT (NO BLOCKING BY ACTIVATION)
function exportData(){
  return {
    license: state.activation || null,
...
// SAFE AUTO LOGIN FLOW (NO LOOP LOCK)
function bootCheck(){
  if(isActivated()){
    currentPage = "login";
  } else {
...
// OVERRIDE SAFE BOOT (DO NOT BLOCK APP)
...
// ===== FINAL EMERGENCY FIX (STATE SAFE EXPORT) =====

function normalizeState(){
  if(!state) state = {};
  state.beneficiariesByMonth = state.beneficiariesByMonth || state.beneficiaries || {};
...
function exportData(){
  ...
  return {
    license: state.activation || null,
    activationStatus: !!state.activated,
    beneficiaries: getAllBeneficiariesForExport(),
    attachments: state.attachments || [],
    documents: state.documents || [],
    exportedAt: Date.now()
...
function exportData(){
  ...
  function addStep(name, value){
    trace.push({step:name, count: Array.isArray(value) ? value.length : (value ? 1 : 0)});
  }

  const license = state.activation || null;
  addStep("مجوز فعالیت", license);

  const beneficiaries = getAllBeneficiariesForExport ? getAllBeneficiariesForExport() : [];
  addStep("لیست مددجوها", beneficiaries);

  const attachments = state.attachments || [];
  addStep("پیوست‌ها", attachments);

  const documents = state.documents || [];
  addStep("مدارک", documents);
  ...
  console.log("EXPORT TRACE:", trace);
...
function openDebugPanel(){
  try{
    const panel = {
      hasState: !!state,
      beneficiariesByMonth: state?.beneficiariesByMonth || {},
      beneficiariesLegacy: state?.beneficiaries || [],
      activation: state?.activation || null,
      stats: {
        months: Object.keys(state?.beneficiariesByMonth || {}).length,
        totalLegacy: (state?.beneficiaries || []).length
      }
    };

    console.log("DEBUG PANEL:", panel);
    alert(JSON.stringify(panel, null, 2));
    return panel;
  }catch(e){
...
function injectDebugButton(){
  try{
    ...
    console.log("DEBUG BUTTON ERROR", e);
  }
}

window.addEventListener("load", injectDebugButton);

// ================= EXPORT UI REPORT FIX =================

function showExportReport(result){
  try{
    ...
    alert(
      "📦 EXPORT DONE\n\n" +
      "🔐 مجوز: " + (result?.license ? "OK" : "EMPTY") + "\n" +
      "👥 مددجو: " + (result?.beneficiaries?.length || 0) + "\n" +
      "📎 پیوست: " + (result?.attachments?.length || 0) + "\n" +
      "📄 مدارک: " + (result?.documents?.length || 0)
    );
...
// wrap exportData safely
(function(){
  const old = exportData;
  exportData = function(){
